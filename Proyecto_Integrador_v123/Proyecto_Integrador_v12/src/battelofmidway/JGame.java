...
public abstract class JGame extends GameLoop {
    ...
    protected AdministradorSonido admin_sound;
    ...
    public JGame(String title, int width, int height){
        jugador = new Jugador();
        ranking = new Ranking();
        admin_sound = new AdministradorSonido();
        ...
        frame.setDefaultCloseOperation(EXIT_ON_CLOSE); //Permite cerrar todas las ventanas una vez iniciado el juego

        if ((fullScreenSupported) && (bFullScreen)) {
            newDisplayMode = new DisplayMode(1128,634, 32, DisplayMode.REFRESH_RATE_UNKNOWN); //ERROR PANTALLA COMPLETA
...
@Override
    ...
    protected void readPropertiesFile(){
        try{
            FileInputStream in = new FileInputStream("src\\battelofmidway\\datos.properties");
    ...
    public AdministradorSonido getAdmin_sound() {
        return admin_sound;
    }
